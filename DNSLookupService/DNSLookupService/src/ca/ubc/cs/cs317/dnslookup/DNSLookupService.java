...
public class DNSLookupService {
    ...
    public Collection<CommonResourceRecord> iterativeQuery(DNSQuestion question)
        ...
        if (!cachedResults.isEmpty()) {
            ans.addAll(cachedResults);
        }

        Collection<CommonResourceRecord> servers = cache.getCachedResults(question);
        if(servers.isEmpty()) {
            servers = cache.getBestNameservers(question);
        }
        for(int i = 0; i < MAX_INDIRECTION_LEVEL_NS; i++) {
            for (CommonResourceRecord ns : servers) {
                InetAddress serverAddy = ns.getInetResult();
                Set<ResourceRecord> RRs = individualQueryProcess(question, serverAddy);
                if(RRs == null) {
                    continue;
                }
                if (RRs != null) {
                    for (ResourceRecord rr : RRs) {
                        ...
                        if (rr.getQuestion().equals(question)) {
                            ans.add((CommonResourceRecord) rr);
                        }
                    }
                }
                i++;
    ...
    public Set<ResourceRecord> individualQueryProcess(DNSQuestion question, InetAddress server)
            throws DNSErrorException {
        /* TODO: To be implemented by the student */
        Set<ResourceRecord> recievedRRs = new HashSet<>();
        DNSMessage query = buildQuery(question);
        int sentTransactionID = query.getID();

        byte[] queryData = query.getUsed();
        DatagramPacket queryPacket = new DatagramPacket(queryData, queryData.length, server,DEFAULT_DNS_PORT);
        try {
            ...
            int attempts = 0;
            boolean respRecieved = false;
            byte[] buff = new byte[MAX_DNS_MESSAGE_LENGTH];
            DatagramPacket resp = new DatagramPacket(buff, buff.length);
            while (attempts < MAX_QUERY_ATTEMPTS && !respRecieved) {
                try {
                    socket.receive(resp);
                    DNSMessage respMsg =  new DNSMessage(resp.getData(),resp.getLength());
                    if (respMsg.getID() == sentTransactionID) {
                        respRecieved = true;
                        recievedRRs = processResponse(respMsg);
                    }
                } catch (Exception e) {
                    attempts++;
                    socket.send(queryPacket);
            ...
            throw new DNSErrorException("Error in processl");
        }
        return recievedRRs;
    }
    ...
    public Set<ResourceRecord> processResponse(DNSMessage message) throws DNSErrorException {
        ...
        if(message.getRcode() != 0 || message.getTC() || !message.getQR()) {
            throw new DNSErrorException("Error Processing the response");
        }
        ...
        for(int i = 0; i < message.getARCount(); i++) {
            ...
            switch (rr.getRecordType()) {
                ...
                case AAAA:
                    ...
                    break;
                default:
                    byte[] rawData = message.getUsed();
                    String hex = message.byteArrayToHexString(rawData);
