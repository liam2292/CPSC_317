...
public class DNSLookupService {
    ...
    public Collection<CommonResourceRecord> iterativeQuery(DNSQuestion question)
        ...
        int indirectLevels = 0;
        while (indirectLevels <= MAX_INDIRECTION_LEVEL_NS) {
    ...
    public Set<ResourceRecord> individualQueryProcess(DNSQuestion question, InetAddress server)
            throws DNSLookupService.DNSErrorException {
        /* TODO: To be implemented by the student */
        ...
        try {
            ...
            while (attempts < MAX_QUERY_ATTEMPTS && !respRecieved) {
                try {
                    ...
                    if (respMsg.getID() == sentTransactionID) {
                        respRecieved = true;
                        recievedRRs = processResponse(respMsg); // Now safe to process the response
                    }
                } catch (Exception e) {
                    attempts++;
                    socket.send(queryPacket);
            ...
            if(!respRecieved) {
                throw new DNSErrorException("No response recieved in the specified amount of attempts");
            }
        } catch (Exception e) {
