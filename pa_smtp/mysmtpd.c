...
int do_mail(smtp_state *ms) {
    ...
    // TODO: Implement this function
    if (ms->state != Ready ) {
        if(send_formatted(ms->fd, "503 Bad sequence of commands\r\n")<=0) return -1;
        return 1;
    }
...
int do_data(smtp_state *ms) {
    ...
    while (nb_read_line(ms->nb, line) > 0) {
        if (strcmp(line, ".\n") == 0 || strcmp(line, ".\r\n") == 0) {
            break;
        }
        char *content = line;
    ...
    close(temp);
    save_user_mail(tempName, ms->users);
    unlink(tempName); 
